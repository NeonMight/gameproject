  ...
  socket.on('init',function(usr) //sends card data to client to be rendered
  {
    ...
    for (var i = 0; i < inRoom[socket.room].length-1; i++) //for each user minus the server
    {
      ...
      for (var j = 0; j < userCards[name].length; j++) //for each card
      {
        ...
        else io.sockets.in('room'+socket.room).emit('card', {x:position+offset/*player offset + card offset*/, y:400, val:userCards[name][j], user:name, type: "hidden"});
...
http.listen(3003, function() {
          console.log('Blackjack Championship Edition node now listening on port 3003');
});
